...
class JsonFeederSimulation extends Simulation {
  ...
  private val feeder = jsonFile("feeders/new_computers.json").circular
  ...
  setUp(scn.inject(rampUsers(usersToBeInjected) during (10 seconds)).protocols(httpConf))
}
